...
public class PaymentApiController extends GridController {
	 ...
	 */
	@ResponseBody
	@RequestMapping(value="/beginselect",produces=MediaType.TEXT_HTML_VALUE)
	public String beginselect(Integer orderId){
	//	boolean isUse=true;
		Order order = this.orderManager.get(orderId);
		...
		//本来的余额
		double  balance=esaccManager.getEsaccById(memberid);
		if(order.getPay_status()!=2){
		if(amount<balance){
			// 查询购买后的余额
			double  gmbalance =CurrencyUtil.sub(balance, amount);
			// 增加一条流水数据
			Esacc esacc=new Esacc();
			esacc.setBill_code(order.getSn());
			esacc.setBill_date(DateUtil.getDateline());
			esacc.setDepartment(member.getAgent_level());
		   esacc.setBill_remark("购买单号为"+order.getSn()+"的商品");
			esacc.setBill_in(0d);
			esacc.setBill_out(amount);
			esacc.setBill_balance(gmbalance);
			esacc.setAccount(order.getRemark());
			esacc.setMember_id(memberid);
			esaccManager.saveAdd(esacc);
